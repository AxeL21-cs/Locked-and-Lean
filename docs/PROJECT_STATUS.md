...
- Deployed migration `20260717031644_goal_weight_target_planner` to hosted Supabase. Hosted history, target columns, invoker/definer function modes, empty search paths, and authenticated execution grants match the reviewed contract; advisors reported no new planner issue.
- Produced the signed Android 0.4.2 (versionCode 6) internal-distribution APK from commit `9422707`, preserving the production package and signing identity for update-in-place installation.
- Resolved the hosted web onboarding contract mismatch. Supabase API logs showed the stale July 16 web bundle saving `upsert_profile` successfully and then receiving HTTP 404 for the retired `propose_nutrition_target` argument set. The database was already correct; production deployment `dpl_BicXRKcifvHk7pBTvEpYsRQi841e` now serves the target-weight client from commit `99f256d`.
- Structured PostgREST errors now preserve their safe message and code. A `PGRST202` function-signature mismatch produces an explicit refresh/update instruction instead of `Unexpected service error`, and SQLSTATE `22023` remains a validation error.
- Added a tracked `.vercelignore` so ignored APKs and local build artifacts cannot exceed Vercel's per-file upload limit. Production public Supabase and hosted-consent values are explicitly scoped in the Vercel project rather than depending on a one-off local build file.
...
- Goal-weight planner database gate: a clean local Supabase reset passed, and the complete database directory passed 359/359 pgTAP assertions across 11 suites. The repository-wide gate passed formatting, Expo lint, TypeScript, 35 Jest suites / 197 tests, 44 static contracts, 43 MCP tests/build, and the prohibited model-API scan. Expo resolved Android 0.4.2/versionCode 6 with only the intended camera permission, and the Android production export bundled successfully. Hosted migration alignment and post-deployment function/column/advisor verification passed.
- Android EAS production build `d1f184ac-5550-4fe0-983d-3342594eb736` finished for version 0.4.2 (versionCode 6) from commit `9422707`. The downloaded 133,333,189-byte APK contains `AndroidManifest.xml`, `resources.arsc`, five DEX files, 104 native-library entries, and 1,384 ZIP entries; SHA-256 is `6C2D505E2A74B0F61D9C31FD07ED7EC54B96A3A0D3C2BD2E0CD2A654826E3007`. Google `apksig` verifies its v2 signature with zero warnings/errors, and signer SHA-256 `6F699A9AEB4CDB9D0D7A3034E760EC12F0738B8B600E44DAEE505AB0BFAF64BD` matches the verified Android 0.4.1 artifact.
- Physical Android 0.4.2 update verification passed on a POCO F7 Pro (`24117RK2CG`) running Android 16: ADB replace-install upgraded `com.lockedandlean.app` from 0.4.0/versionCode 4 to 0.4.2/versionCode 6 while preserving the original `firstInstallTime` and app storage. A cold launch opened the authenticated Today screen as the foreground activity in 965 ms, and filtered startup logs contained no fatal, Android runtime, React Native, Expo, or ANR error.
- Public GitHub Release `v0.4.2` targets source commit `9422707` and provides the signed `Locked-and-Lean-0.4.2-build-6.apk` as an Android package asset. GitHub reports the uploaded asset as 133,333,189 bytes with digest `sha256:6c2d505e2a74b0f61d9c31fd07ed7ec54b96a3a0d3c2bd2e0cd2a654826e3007`, matching the locally verified artifact.
- The public README now leads with the Android download and web preview, explains the product and exact preview-confirm flow in user language, includes Android installation and local-development paths, and documents the restricted ChatGPT developer-preview connection without presenting it as a publicly listed plugin. The APK, release, web preview, MCP, and official OpenAI instruction links were checked against their live endpoints; GitHub's GFM renderer accepted the README; and `npm run check` passed formatting, Expo lint, TypeScript, 35 Jest suites / 197 tests, 44 static contracts, 43 MCP tests/build, and the prohibited model-API scan.
- Web onboarding incident gate: `npm run check` passed formatting, Expo lint, TypeScript, 36 Jest suites / 200 tests, 44 static contracts, 43 MCP tests/build, and the prohibited model-API scan. The production deployment is `READY`, the canonical alias resolves to it, the served bundle contains `Build a realistic target`, `p_target_weight_kg`, the Supabase project ref, and the compatibility message, and it does not contain the retired `p_goal` RPC argument. The unauthenticated production sign-in page loaded without browser errors; one React Native Web animation fallback warning remains.
...
- Physical Android launch, update-in-place, session preservation, and light/dark rendering are verified on the connected POCO F7 Pro. The device denied ADB `WRITE_SETTINGS` and input injection, so 200% system-font layout, TalkBack traversal, draft-back confirmation, offline/reconnect, and low-end frame profiling still require manual interaction on the phone.
- The 0.4.2 artifact is signed, archive-validated, and physically verified as an update-in-place installation on the connected POCO F7 Pro. Manual target-form interaction, large-font/TalkBack traversal, offline/reconnect behavior, and a representative confirmed meal still need user-driven device checks.
