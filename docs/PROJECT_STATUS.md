...
- The 0.4.2 artifact is signed, archive-validated, and ready for update-in-place installation. Google platform tools were installed temporarily, but Windows currently enumerates no Redmi, Android, MTP, or ADB device, so installation and the 0.4.2 physical smoke test remain pending until USB mode/debug authorization is restored.
...
2. Install Android 0.4.2 over the existing app once Windows enumerates the phone through ADB, then verify the remembered setup, target review/confirmation, Today target refresh, and session preservation.
