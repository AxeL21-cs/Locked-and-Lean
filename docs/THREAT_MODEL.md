...
| T-02 | Client supplies another `user_id`.                                                          | RPC schemas omit owner; database derives `auth.uid()`; reject owner claims at transport.                                                                              | Confirmation and ChatGPT preview/revision accept no `user_id` and derive `auth.uid()`. Local pgTAP rejects cross-owner revision and confirmation.                                                                                                                                                                                                |
| T-03 | Client submits favorable calorie or macro totals.                                           | Accept item facts only through a validated preview path; recompute current item and entry totals in the transaction.                                                  | ChatGPT preview/revision accept no aggregate totals, validate bounded item facts, force estimate provenance/uncertainty, and sum totals in PostgreSQL. Confirmation recomputes from the stored current revision; local pgTAP covers both boundaries.                                                                                             |
...
| T-06 | OAuth token performs an action not consented/allowed.                                       | Verify issuer/audience/expiry/client at MCP; enforce a server-owned client/action policy at the database; enforce granular scopes only when the issuer supports them. | MCP verifies signature, issuer, audience, validity, role, subject, and exact configured client/action. PostgreSQL repeats exact action checks for preview, revision, and confirmation. Standard Supabase scopes still do not express food/weight permission, so access remains restricted and general ChatGPT writes remain blocked by ADR-0001. |
