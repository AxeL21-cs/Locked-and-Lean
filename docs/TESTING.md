...
Supabase OAuth still does not support custom food/calorie/weight scopes. The consent UI therefore labels standard scopes as identity-only and blocks unsupported scopes. `preview_food_log` and general preview revision authenticate and then return `backend_contract_unavailable` with `permanent_write: false` because no OAuth-compatible backend RPC exists; they do not call an incompatible RPC or create a permanent entry. General production MCP writes remain blocked under ADR-0001.
