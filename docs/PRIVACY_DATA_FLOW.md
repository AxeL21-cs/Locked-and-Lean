...
Preview, revision, and confirmation require token-derived `client_id` values with separately enabled exact action rows and deny missing or unknown clients. The MCP verifier checks signature, exact issuer, canonical resource audience, time claims, subject, role, and `client_id` before forwarding the user's bearer token. Local tests cover approved, missing, wrong-action, and cross-user cases; hosted revocation and post-expiry behavior still require end-to-end verification. Supabase's current standard OAuth scopes describe identity output rather than granular food/weight permissions, so this remains a restricted allowlisted flow rather than general production authorization under ADR-0001.
...
| ChatGPT                          | user-provided description/image and returned preview    | user-facing interpretation boundary; restricted OAuth MCP preview/revision/confirmation is implemented |
